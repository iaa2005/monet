...
app.whenReady().then(async () => {
  ...
  // aria-label, not title: IconBtn splits "Files — Ctrl+Shift+E" into a
  // Hint tooltip and an aria-label, and stopped setting `title` at all.
  // This probe went on selecting by title and quietly failed for it.
  const hasBtn = await js(`!!document.querySelector('button[aria-label="Files"]')`);
