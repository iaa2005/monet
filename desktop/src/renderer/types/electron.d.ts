...
import type { PermissionRequest, PermissionDecision } from '../main/ipc/permissions.js'
...
export interface ElectronAPI {
  ...
  files: {
    ...
    list: (dirPath: string) => Promise<{ name: string; isDirectory: boolean; isFile: boolean; path: string }[]>
    exists: (path: string) => Promise<boolean>
    pickDirectory: () => Promise<string | null>
  }
  shell: {
    run: (command: string, cwd?: string) => Promise<{ ok: boolean; stdout: string; stderr: string; error?: string }>
  }
  providers: {
    ...
    update: (id: string, input: Partial<LLMProviderInput>) => Promise<LLMProvider | null>
