...
/**
 * Download: the tray, with the arrow going into it.
 *
 * For the update row, and picked by the user from hugeicons — kept at that
 * set's 1.5 because the drawing is built for it: at 2 the arrowhead inside
 * the tray closes into a wedge. `currentColor` throughout is the point of it
 * here: the row wanted an icon the same colour as its own text, black on
 * white and white on black, not a tinted badge.
 */
export function Download05(props: SVGProps<SVGSVGElement>): JSX.Element {
  return (
...
/**
 * Python: the two snakes.
 *
 * The one icon here that is NOT at stroke 2. Where the snakes interlock, the
 * gap between them is about a stroke wide, so at 2 the middle of the mark
 * closes into a solid block at 16px — the size the settings list draws it at.
 * 1.75 keeps the gaps open and still reads at the same weight as its
 * neighbours; 1.5 was lighter than the row around it.
 *
 * The eyes are filled, not stroked — hence the fill/stroke override on them.
 */
export function PythonIcon(props: SVGProps<SVGSVGElement>): JSX.Element {
  return (
    <Icon strokeWidth="1.75" {...props}>
      <path d="M12 7.5H7.5m4.5 9h4.5m0 0h1.521c.807 0 1.634-.188 2.13-.824c.531-.679 1.099-1.835 1.099-3.676c0-1.84-.568-2.997-1.098-3.676c-.497-.636-1.324-.824-2.13-.824H16.5m0 9v1.521c0 .807-.188 1.634-.824 2.13c-.679.531-1.835 1.099-3.676 1.099c-1.84 0-2.997-.568-3.676-1.098c-.636-.497-.824-1.324-.824-2.13V16.5m0-9H5.978c-.807 0-1.633.188-2.13.824c-.53.679-1.098 1.835-1.098 3.676c0 1.84.568 2.997 1.098 3.676c.497.636 1.323.824 2.13.824H7.5m0-9V5.978c0-.807.188-1.633.824-2.13c.679-.53 1.835-1.098 3.676-1.098c1.84 0 2.997.568 3.676 1.098c.636.497.824 1.323.824 2.13V7.5m-9 9V14a2 2 0 0 1 2-2h5a2 2 0 0 0 2-2V7.5" />
      <path
        fill="currentColor"
        stroke="none"
...
/** Node.js: the hexagon, with JS inside it. */
export function NodeIcon(props: SVGProps<SVGSVGElement>): JSX.Element {
  return (
    <Icon {...props}>
