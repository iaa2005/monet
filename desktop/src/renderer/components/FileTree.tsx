...
function TreeNode({ entry, depth, onSelectFile }: {
  entry: FileEntry
  depth: number
  onSelectFile?: (path: string) => void
}): JSX.Element {
  ...
  const handleToggle = async (): Promise<void> => {
    ...
    if (!expanded && !children) {
      ...
      try {
        ...
        setChildren(items.filter(e => !e.name.startsWith('.')).sort((a, b) => {
  ...
  return (
    <div>
      ...
      {expanded && loading && (
        <div style={{ paddingLeft: `${(depth + 1) * 16 + 4}px` }} className="text-xs text-muted-foreground py-0.5">
          Loading...
        </div>
      )}
