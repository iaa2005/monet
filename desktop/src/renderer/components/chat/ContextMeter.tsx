 ...
 * prompt, tool schemas, MCP, free) with token counts and percentages.
 *
 * The breakdown is estimated (chars/4) from the exact pieces the agent sends,
 * so it shows the *mix*, not an exact accounting.
 */
...
export function ContextMeter({
  ...
  const load = async (): Promise<void> => {
    setLoading(true);
    try {
      const r = await api()?.chat.contextBreakdown(sessionId ?? "default", space);
      if (r) setData(r);
    } catch {
      /* best-effort */
    } finally {
  ...
  const pct = data ? Math.round((data.used / data.budget) * 100) : ctxPct;
    ...
    >
      ...
      <DropdownMenuContent side="top" align="end" className="w-80 p-3">
        ...
        <div className="mt-2.5 border-t border-border pt-2 text-[10px] leading-snug text-muted-foreground">
          Estimated (~chars/4) from the system prompt, tool schemas and
          conversation. Skills and memory are counted within the system prompt.
        </div>
      </DropdownMenuContent>
    </DropdownMenu>
  );
