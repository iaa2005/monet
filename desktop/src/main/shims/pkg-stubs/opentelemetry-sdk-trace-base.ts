/**
 * STUB for '@opentelemetry/sdk-trace-base' — package is Anthropic-internal, cloud-provider or
 * native-only; the desktop build never takes these code paths.
 * Function calls return undefined so feature checks (isXEnabled(),
 * isSupported()) read as disabled; constructing a class throws loudly
 * because a fake client must not silently exist.
 */
/* eslint-disable @typescript-eslint/no-explicit-any */
const stubFn: any = new Proxy(function stubbed() {}, {
  get: (t: any, p: any) => (p in t ? t[p] : stubFn),
  apply: () => undefined,
  construct: () => { throw new Error('stubbed package: @opentelemetry/sdk-trace-base') },
})

export const BasicTracerProvider: any = stubFn
export type BasicTracerProvider = any
export const BatchSpanProcessor: any = stubFn
export type BatchSpanProcessor = any
export const ConsoleSpanExporter: any = stubFn
export type ConsoleSpanExporter = any
export default stubFn
