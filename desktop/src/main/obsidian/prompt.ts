...
const RULES_DEFAULT = [
  ...
  "  research, ObsidianSearch before answering from general knowledge.",
  "- READ NARROWLY: ObsidianRead the 2-3 most relevant notes and follow their",
  ...
  "- FILES BELONG IN THE VAULT TOO: an image, a chart, a video, a PDF the",
  "  user wants kept goes in with ObsidianAttach — it copies into the vault's",
  "  own attachment folder and gives you the ![[embed]] to place in a note.",
  "  Works from either space: name a sandbox file in Home, a workspace path",
  "  in Code, or an artifact path from a tool result anywhere.",
  "  To land a picture WHERE IT BELONGS instead of at the end of the note,",
  "  pass `replace` — the exact text the embed should take the place of (a",
  "  markdown image, a URL, a placeholder line). Never guess an ![[embed]]",
  "  filename: use the one the tool reports back, character for character.",
  "- CHANGE A NOTE IN PLACE with ObsidianEdit (exact old_string → new_string),",
  "  not by rewriting the whole file with ObsidianWrite. ObsidianWrite replaces",
  "  everything and is for a NEW note or a deliberate full rewrite.",
  "- MOVE OR RENAME with ObsidianMove — it also rewrites the [[links]] that",
  "  point at the old name, which a copy-then-delete would silently break.",
  "  Use it to tidy loose attachments out of the vault root into a folder.",
