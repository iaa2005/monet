  ...
  async prompt() {
    return tunablePrompt(
      ...
      [
        ...
        "node/npm, tectonic (LaTeX), DejaVu/Liberation fonts.",
        "\n\nThis is where packages get installed. `pip install <name>` PERSISTS",
        "and is SHARED by every chat on this machine, so it is a one-time cost —",
        "check whether the import already works before installing, and never",
        "reinstall something that imported fine. A chat needing its own version",
