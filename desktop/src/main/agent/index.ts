...
const CHART_WIDGET_DEFAULT = [
  "You can DRAW a chart in an answer: a fenced block whose language is chart,",
  "holding JSON. It is part of the reply, so it costs no tool call and cannot",
  "fail — line, area, bar or candlestick, from numbers you already have.",
  "",
  "Run the /chart skill before writing one. It carries the shape of the JSON,",
  "when a candlestick is the honest chart and a line is not, and how to point",
  "the block at a file instead of retyping a hundred rows. Do not guess the",
  "format: a block the renderer cannot read shows the reader raw JSON, which",
  "is worse than the table you did not write.",
].join("\n");
