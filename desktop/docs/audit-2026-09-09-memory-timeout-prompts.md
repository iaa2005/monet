...
### Отрицательный результат: порядок внутри тулсета не помогает

Напрашивалось: класть раскрытый инструмент в **конец** массива, чтобы схемы
перед ним остались байт-в-байт и сервер переиспользовал префикс до него.
Измерено — не работает:

| раскрытый инструмент | переиспользовано |
|---|---:|
| вставлен на своё место в середине | 23 / 1708 |
| добавлен последним | 23 / 1708 |

Одинаково. Блок инструментов рендерится отдельным системным сообщением
впереди всего, и частичное совпадение внутри него этот сервер не
переиспользует. Правка была написана и **откачена**; комментарий в
`vendor-tools.ts` остался, чтобы её не написали снова.

Что работает — это где стоит изменчивый ТЕКСТ, а не порядок внутри тулсета.
