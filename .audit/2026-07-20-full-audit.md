...
В исходной сессии реализовывались только фичи из технического задания — багфиксы из §1-4 были оставлены для отдельного этапа. Они закрыты последующей серией коммитов в `main`; актуальный статус приведён в §8.
...
## 8. Статус аудита после серии исправлений

Пункты §1–§4 и §8.1 обработаны поэтапно отдельными коммитами в `main`:

- `95e89a7` — порог desktop-autocompaction стал model-aware;
- `68e6523` — global process handlers и bounded Electron renderer recovery;
- `55b24f4` — caps/async IO/guards для больших файлов, подсветки и diff;
- `c865541` — валидация routine output/grants и единый connector scope;
- `b1d1665` — полноценный MIME parsing через `mailparser` + Yandex hint;
- `54dba47` — локальный viewer ErrorBoundary и безопасные ошибки files IPC;
- `929a738` — connector-aware routine drafts и точный `SKIP` protocol.

### 8.1 Автокомпрессия desktop-agent — исправлено

Корень проблемы был в абсолютном `DEFAULT_THRESHOLD = 150_000` в desktop-owned [main/agent/compaction.ts](../desktop/src/main/agent/compaction.ts). Даже при `contextLimit = 1_000_000` функция ограничивала порог 150K.

Теперь:

- `inputLimit` используется напрямую, если провайдер его сообщает;
- иначе из `contextLimit` вычитается резерв под output (`maxTokens`, fallback 16K);
- сохраняется safety factor 70%;
- `MONET_COMPACT_TOKENS` остаётся явным override;
- Caveman mode по-прежнему намеренно снижает готовый порог до 60%.

Для DeepSeek с `contextLimit = 1M` и резервом 16K порог теперь примерно `688.8K`, а не 150K. Это устраняет прежний ранний trigger. Решение по-прежнему использует desktop-эвристику `chars / 4`, а не tokenizer/provider usage; hybrid measured-usage baseline оставлен отдельным улучшением после runtime-измерений.

### 8.2 Оставшиеся ограничения, не являющиеся незакрытыми аварийными багами

1. `files:readBytes` всё ещё передаёт бинарные previews через base64. Переход на transferable/structured-clone `ArrayBuffer` не делался, чтобы не менять IPC-контракт без отдельного измерения.
2. Yandex/обычный IMAP search сохраняет безопасный простой поиск по subject/body/from; полноценный Gmail-like язык (`has:attachment`, `before`, `after`, `is:unread`) не добавлялся частично и требует отдельного структурированного API.
3. Routine skip сохраняет статус `skipped`, но отдельная model-generated причина skip не добавляется; structured result для `runAgent` намеренно не вводился.
4. В проекте нет полноценного unit/integration test runner для MIME, IPC, Electron crash recovery и routine scheduler. Проверка остаётся на typecheck/build/smoke и ручных Electron-сценариях.
5. `npm --prefix desktop run typecheck` сейчас сообщает две существующие ошибки вне этой серии: `ChatStore.requestOpenChanges` отсутствует для вызовов в `renderer/App.tsx` и `renderer/components/chat/GitCard.tsx`. Это отдельный старый дефект, не вызванный текущими коммитами.

### 8.3 Проверка серии

- `npm --prefix desktop run build` — **успешно**.
- `npm --prefix desktop run typecheck` — **неуспешно из-за двух перечисленных старых ошибок**; изменённые файлы этой серии дополнительных ошибок не добавили.
- `git status` после коммитов — рабочее дерево чистое.
- Динамические import warnings Vite остаются информационными и не блокируют сборку.
