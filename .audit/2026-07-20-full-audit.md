...
Отказоустойчивость (§1), производительность больших файлов (§2), Yandex Mail MIME-парсер (§3) и Routines (§4) — по явной просьбе пользователя это НЕ багфикс-сессия, только реализация запрошенных фич (§7). Рекомендуемый порядок при следующем заходе:

1. Отказоустойчивость (§1) — дёшево, снимает краши.
2. Yandex Mail (§3) — точечный фикс (`mailparser`), закрывает конкретную жалобу пользователя.
3. Perf больших файлов (§2) — самый заметный эффект для пользователя, но больше кода.
4. Routines UX (§4) — connector-aware draft + grants в UI, больше работы (данные + UI).
