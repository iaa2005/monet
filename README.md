...
The app targets **Windows and macOS**. `npm install` rebuilds
`better-sqlite3` for Electron's ABI automatically (`postinstall`); if that
step is skipped or fails, run `npm run rebuild:native` — without it anything
touching sessions dies with `ERR_DLOPEN_FAILED`. On macOS, Computer Use also
compiles its Swift helper with the Xcode Command Line Tools on first use
